...
export default function Login() {
  ...
  return (
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <div className="hidden lg:block lg:w-[50%] xl:w-[45%] h-full relative">
        <img
          src="./images/loginLeftImg.png"
          alt="BDB Voting"
      ...
      <div className="flex h-full w-full flex-col items-center justify-center overflow-y-auto p-4 lg:w-[50%] xl:w-[55%]">
        ...
        <div className="w-full max-w-md space-y-6 p-4 sm:p-8">
          ...
          <div className="space-y-2 text-center">
            ...
            <h1 className="text-xl font-extrabold tracking-tight text-slate-900 text-left">BDB Voting</h1>
            <p className="text-xs text-slate-500 text-left">Sign in to access elections management</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div>
              ...
              <select
                value={year}
                onChange={(e) => setYear(e.target.value)}
                className="w-full cursor-pointer rounded-xl border border-slate-300 bg-slate-50 p-2.5 font-semibold text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {FY_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-3 flex w-full cursor-pointer items-center justify-center space-x-2 rounded-xl bg-blue-600 py-3 text-sm font-bold text-white shadow-md transition-all hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              <span>{loading ? "Signing in…" : "Sign In"}</span>
              {!loading && (
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
