      ...
      <div className="flex items-center justify-between border-b border-slate-200/80 px-3.5 py-2.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className={`rounded px-2 py-0.5 font-mono text-[10px] font-bold ${
            locked ? "bg-slate-200 text-slate-500" : blocked ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-800"}`}>
            {locked ? "ALLOTTED" : blocked ? "BLOCKED" : "OPEN"}
          </span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {!locked && (
            <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${!blocked ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-700"}`}>
              {!blocked ? "✓ Eligible" : "✗ Not eligible"}
            </span>
          )}
        </div>
      ...
      {locked ? (
        <div className="border-t border-slate-200 bg-slate-50 px-3.5 py-2.5">
          <div className="flex items-center justify-between">
            ...
            {code.allotted_at && (
              <span className="font-mono text-[10px] text-slate-400">{new Date(code.allotted_at).toLocaleString()}</span>
            )}
          </div>
        </div>
      ) : blocked ? (
