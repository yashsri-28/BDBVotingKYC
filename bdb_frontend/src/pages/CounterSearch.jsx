            ...
            <div className="flex flex-col items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 p-4 sm:flex-row">
              ...
              <div className="flex w-full items-center space-x-3 sm:w-auto">
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 rounded-lg px-5 py-2 text-xs font-bold transition-all ${
                    selected.size > 0 && !isArchiveYear ? "bg-emerald-600 text-white shadow-md hover:bg-emerald-700" : "cursor-not-allowed bg-slate-300 text-slate-500"
                  }`}
                >
...
function BallotCodeCard({ code, selected, onToggle, disabled }) {
  ...
  return (
    <div
      className={`relative flex flex-col justify-between space-y-3.5 rounded-xl border p-4 transition-all ${
        locked
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
        ...
        {code.roll_type && (
          <span className={`rounded-lg border px-3 py-1 text-xs font-bold uppercase tracking-wide shadow-2xs ${
            code.roll_type === "category" ? "border-purple-200 bg-purple-100 text-purple-800" : "border-amber-200 bg-amber-100 text-amber-800"
          }`}>
            {code.roll_type} Pool
          </span>
        )}
      ...
      <div className="space-y-1 rounded-lg border border-purple-100 bg-purple-50/60 p-2.5">
        <div className="flex items-center justify-between text-[11px]">
          ...
          <span className="font-bold text-purple-950">{code.representative_name || "—"}</span>
        </div>
        <div className="flex items-center justify-between text-[11px]">
          <span className="flex items-center gap-1 font-bold text-purple-700">
            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
